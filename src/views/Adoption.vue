    ...
    <b-container fluid>
      <div class="adoption-form">
        <form v-if="userInfo.admin == true">
          ...
          <input
            ...
            class="inputfile"
          />
          <label id="label" for="select-img">Choose a file</label>
          <b-button id="upload-button" v-on:click.prevent="uploadFile">
            Upload
          </b-button>
        </form>
      </div>
      <div class="dog-box" v-if="dogs.length > 0">
        <div v-for="dog in dogs" :key="dog.id">
          <b-card
            ...
            img-top
          >
            <h4 slot="header" class="dog-name">{{ dog.name }}</h4>

            <b-card-body>
              <b-card-title class="adopt-stat">{{
                dog.adoption_status
              }}</b-card-title>
            </b-card-body>
      ...
      >
        <div class="dog-card" v-if="adoptform == true">
          <p class="dog-card-title">Info Sheet</p>
          <img
            class="modal-img"
            :src="modalDog.url"
            alt=""
